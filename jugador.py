...
class Jugador:
    ...
    def __init__(self,color):
        ...
        if self.color=='Red':           #Estas variables se usan para saber la distancia 
            self.last_spot=0
    ...
    def spawn(self):
        ...
        for i in self.piezas:
            if i.spot==0:                    #Se asegura de que la pieza que va a sacar esta en start
                self.start-=1
                self.path+=1
                if self.color == 'Red':       #Se asegura del color del jugador para spawn a su pieza en
    ...
    def move(self,ds,i):
        ...
        '''
        if self.piezas[i].spot==0:                                      #Asegura que la pieza no este en start
            pass
        
        elif (self.piezas[i].spot== 72 or self.piezas[i].spot== 80 or   #Asegura que la pieza no este en end
        self.piezas[i].spot== 88 or self.piezas[i].spot== 96):
            pass

        else:
            spot_nuevo=self.piezas[i].spot + ds                         #Se anade 'ds' a el spot de la pieza 'i' en 
                                                                        #   en una variable aparte para verificar
            if self.color=='Red':                                       #Como el ultimo spot de 'Red' es 64
                pass #nada                                              #   no hay que hacer el ciclo
            else:
                if spot_nuevo > 64:                                     #Ciclo circular de los spots
                    spot_nuevo=spot_nuevo-64

            if self.color=='Red' and spot_nuevo > 64:                   #Verificaciones para chequear si la pieza esta
                self.end_spot_verificacion(72, spot_nuevo,i)            #   en el end path
            
            elif self.color=='Blue' and spot_nuevo > 16:
                spot_nuevo = 16 - self.piezas[i].spot + 72
                self.end_spot_verificacion(80, spot_nuevo,i)

            elif self.color=='Yellow' and spot_nuevo > 32:
                spot_nuevo = 32 - self.piezas[i].spot + 80
                self.end_spot_verificacion(88, spot_nuevo,i)
            
            elif self.color=='Green' and spot_nuevo > 48:
